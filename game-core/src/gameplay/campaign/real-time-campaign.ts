...
import { GAME_CONFIG } from "../../config/generated/game-config.generated.ts";
...
const campaignConfig = GAME_CONFIG.balance.campaign;
...
export const BUILDING_CATALOG: Record<BuildingKind, {
  ...
  buildSeconds: number;
  description: string;
}> = campaignConfig.buildings;

export const RESEARCH_CATALOG: Record<ResearchKind, { name: string; gasCost: number; durationSeconds: number }> = campaignConfig.research;
...
export class RealTimeCampaign {
  ...
  public getDeliveryCapacity(): number {
    return campaignConfig.simulation.baseDeliveryCapacity
      + this.getBuildingCount("delivery-center") * campaignConfig.simulation.deliveryCenterCapacity;
  }
  ...
  public update(dt: number, nodes: ReadonlyArray<MapNode>): { gasIncome: number; completed: CampaignTimedJob[] } {
    const safeDt = Math.max(0, Math.min(campaignConfig.simulation.maxUpdateSeconds, dt));
    this.elapsedSeconds += safeDt;
    const refineryIncome = this.getBuildingCount("refinery") * campaignConfig.simulation.refineryIncomePerSecond;
...
export function getNearestLogisticsSource(nodes: ReadonlyArray<MapNode>, battlefieldNodeId: string): { id: "home" | string; name: string; distance: number } {
  ...
  const targetDistance = Math.max(0, target?.distanceFromHome ?? campaignConfig.logistics.defaultDistance);
...
export function quoteBattleLogistics(nodes: ReadonlyArray<MapNode>, battlefieldNodeId: string, unitSpeed: number, templateId?: number): LogisticsQuote {
  ...
  const travelSeconds = freeFromOutpost
    ? campaignConfig.logistics.outpostTravelSeconds
    : Math.max(
      campaignConfig.logistics.minTravelSeconds,
      (distance * campaignConfig.logistics.distanceSecondsFactor) / Math.max(campaignConfig.logistics.minUnitSpeed, unitSpeed),
    );
  return {
    ...
    distance,
    travelSeconds,
    gasCostMultiplier: freeFromOutpost
      ? 0
      : 1 + Math.min(campaignConfig.logistics.maxDistanceCost, distance / campaignConfig.logistics.distanceCostDivisor),
    freeFromOutpost,
  };
}
