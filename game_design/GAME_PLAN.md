...
- Built-in composed choices are the `L1` through `L5` ladder. `L1` is the former L2 capability-aware target/movement/shoot bundle. The remade L2-L5 combine property-derived role routing, loaded-ammo attack/retreat cycles, range control, urgent projectile evasion, ally spacing, defensive pressure, time-to-disable target ranking, weak-cell aiming, history/acceleration-aware intercepts, and per-weapon compatible-target selection. Fast aircraft with deep magazines are classified as raiders and alternate attack runs with reload retreats; aircraft that do not meet those live property thresholds keep ordinary engagement behavior.
...
- Level 5 keeps its strategic target for movement and pursuit. Each ready weapon independently checks reachability using its own range, firing axis, projectile speed, and relative target motion. A weapon temporarily redirects when the strategic target is infeasible or when a much closer reachable enemy's decisive control support can be disabled with the weapon's currently loaded magazine. Other weapons may continue engaging the strategic target, and no craft or part identity is consulted.
