...
- `npm --prefix arena run eval:levels -- --minLevel 2 --maxLevel 5 --seeds 128 --threshold 0.6` runs one deterministic fixed-orientation `p4-leaderboard` match per seed and fails unless every higher adjacent level wins at least 60% of held-out matches; ties do not count as wins.
...
- Leaderboard competition, leaderboard-nearby training, and headless AI evaluation share one `loadLeaderboardScenario()` path. They use Test Arena's authored battlefield (`3000x1500`, `600` ground height), movement/simulation defaults, node defense (`1.1`), and base HP (`1,000,000,000`). Comparison-only rules select from every valid non-base runtime craft. At game time zero and every configured interval thereafter, one craft is selected randomly and the same six copies are deployed free for each side. Defaults are six craft per wave, a 20-second interval, and a 100-second game-time duration.
- The AI Global Settings category owns comparison spawn count, spawn interval, simulation duration, and base worth. A comparison continues for the full duration after a base is destroyed. Destroying a craft is worth one destroyed unit and destroying a base is worth 20; no gas, surviving-integrity, remaining-population, or base-HP tiebreaker is used.
...
- `npm --prefix arena run verify:ai-rules` guards the nearby-enemy base suppression, destroyed-unit-only single-match scoring, authored comparison settings, and shared Test Arena/leaderboard battlefield defaults.
