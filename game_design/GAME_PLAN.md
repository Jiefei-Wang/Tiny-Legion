...
- Built-in composed choices are the explicit certified `L1` through `L5` ladder. Each level has its own certified target and movement family: local engageable threats lead the per-weapon fire list while a short committed strategic target independently drives movement. Shared movement doctrine allows border transit but applies soft recovery after prolonged dwell, holds facing/direction changes, makes ground craft prefer vertical projectile dodges with limited reverse movement, gives aircraft full-vector evasion, and keeps nearby enemies relevant to movement. Higher levels improve reaction, compatibility ranking, multi-threat prediction, and coordination. Every certified shoot component continues to delegate to the unchanged `unified-shoot` intercept core and retains aggregate accuracies of 50%/60%/70%/80%/90%.
