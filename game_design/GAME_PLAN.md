...
- Craft Arena definitions, global settings, and latest results auto-save locally. The middle pane presents a craft-by-craft heat map with top tabs for destroyed count and authored gas wasted; each cell reports the row craft's value and uses a diverging color to show whether it lost less or more than its opponent. Selecting a cell opens both sides' detailed statistics and a single-matchup run action in the right inspector. Craft Arena draws no battle canvas and does not modify leaderboard ratings.
...
- Built-in composed choices are the `L1` through `L5` ladder. Their target and movement modules retain property-derived role routing, range control, evasion, spacing, and preservation tactics, while every certified shoot component now delegates to the same `unified-shoot` intercept core. L1-L5 apply per-shooter weighted target-velocity observation plus a stable target-relative near-miss profile to produce the required aggregate all-weapon accuracies of 50%/60%/70%/80%/90%. Miss error clears the target hit cone by a small geometry-derived margin, stays on one side for a shooter/target pair, and becomes hesitation when a close-range miss would require more than 10 degrees of angular error; the standalone `unified-shoot` option exposes the exact core without an intentional accuracy penalty.
...
- `npm --prefix arena run eval:levels -- --minLevel 2 --maxLevel 5` runs exactly one deterministic fixed-orientation `p4-leaderboard` match for each requested level against its immediately previous level. The symmetric battlefield does not require a side-swapped rematch. A higher level passes only when its weighted destroyed-craft total divided by the previous level's total (with a denominator floor of one) is at least `1.10`.
...
- Leaderboard model pool contains only the six certified built-ins (`level-1-ai` through `level-6-ai`). Saved training artifacts remain available in AI Selection but are not silently duplicated as built-ins. The persistent version-11 store invalidates ratings from older comparison rules, records Elo, global W/L/T, pair rounds, per-pair results, weighted destroyed/lost totals, and per-match destroyed ratios. The UI shows average destroyed ratio and cumulative destroyed/lost totals for every AI.
