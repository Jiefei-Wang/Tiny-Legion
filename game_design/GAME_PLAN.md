...
- `npm --prefix arena run eval:levels -- --minLevel 2 --maxLevel 5 --seeds 128 --threshold 0.6` runs the real `p4-leaderboard` scenario as deterministic, two-game side-swapped series and fails unless every higher adjacent level wins at least 60% of held-out series; ties do not count as wins.
...
- Leaderboard competition and headless AI evaluation use the `p4-leaderboard` settings from `arena/composite-training.phases.json`. They share Test Arena's authored battlefield (`3000x1500`, `600` ground height), node defense (`1.1`), and effectively indestructible bases (`1,000,000,000` HP). Comparison-only rules select from every valid runtime craft, start 10 per side, and immediately replenish each side for free to 10 operational craft throughout the finite `120s` evaluation. Initial rosters shuffle the complete filtered catalog and use distinct craft before repeating; replacements remain property/catalog driven without preferred IDs.
- One leaderboard round is a side-swapped pair on the same seed. The pair is scored once using ordered destroyed gas value, destroyed craft count, surviving unit integrity, operational units, base HP, and gas-worth margins. Craft combat therefore decides the result before base damage, preventing base-rush certification or favorable-side credit.
...
- Leaderboard model pool contains only the six certified built-ins (`level-1-ai` through `level-6-ai`). Saved training artifacts remain available in AI Selection but are not silently duplicated as built-ins. The persistent version-8 store invalidates ratings from the earlier base-first score and level definitions, records Elo, global W/L/T, pair rounds, and per-pair results, and lets the UI show each level's saved win rate versus the previous level with a certification mark after 16 rounds above 60%.
- `npm --prefix arena run verify:ai-rules` guards the nearby-enemy base suppression, craft-first mirrored score ordering, and shared Test Arena/leaderboard battlefield defaults.
