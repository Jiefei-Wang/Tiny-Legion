...
- Craft Arena is a renderer-free balance laboratory for saved craft-vs-craft scenarios. Its left pane owns one global battle duration (four minutes by default), shared composed AI, and Test Arena battlefield width/height/ground-height settings. Each expandable matchup row owns the Craft A/B identities and quantities.
- Running a Craft Arena matchup executes one continuous battle: Craft A is always Player on the left and Craft B is always Enemy on the right. No bases or gas spending are used. A destroyed craft respawns immediately and for free, keeping each side at its authored scenario quantity until the duration expires.
- Craft Arena scenario definitions, global settings, and latest results auto-save locally. The collapsed row reports only each craft name/quantity, destroyed count, and authored gas value wasted; expanding it reveals configuration and actions. Craft Arena draws no battle or graph and does not modify leaderboard ratings.
