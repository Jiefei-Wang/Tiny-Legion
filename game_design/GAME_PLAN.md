...
- Level 5 keeps its strategic target for movement and pursuit. Each ready weapon independently checks whether that target is reachable using its own range, firing axis, projectile speed, and relative target motion. Only a weapon with no feasible intercept temporarily fires at a reachable nearby threat; other weapons may continue engaging the strategic target, and no craft or part identity is consulted.
...
- Leaderboard competition runtime uses `p4-leaderboard` settings from `arena/composite-training.phases.json`. Its `useGlobalBattlefield` flag resolves the same authored battlefield defaults used by Test Arena (`3000x1500`, `600` ground height), while the phase retains four initial units per side, every valid craft in the runtime-merged part/template catalog, fixed zero reinforcement gas, a `120s` limit, and `1200` base HP. Each starter group shuffles the filtered catalog and uses distinct craft before repeating, rather than preferring hard-coded IDs.
