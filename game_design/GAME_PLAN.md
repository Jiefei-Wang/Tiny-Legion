...
- Craft Arena is a renderer-free balance laboratory for every distinct craft pairing. Its left pane owns global craft quantity per side, battle duration (four minutes by default), shared composed AI, Test Arena battlefield width/height/ground-height settings, and the action to rerun all matchups.
- Running a Craft Arena matchup executes one continuous battle: Craft A is always Player on the left and Craft B is always Enemy on the right. No bases or gas spending are used. A destroyed craft respawns immediately and for free, keeping both sides at the global quantity until the duration expires.
- Craft Arena definitions, global settings, and latest results auto-save locally. The middle pane presents a craft-by-craft destroyed-count heat map; each cell reports the row craft's losses and uses a diverging color to show whether it lost fewer or more craft than its opponent. Selecting a cell opens both sides' destroyed statistics and a single-matchup run action in the right inspector. Craft Arena draws no battle canvas and does not modify leaderboard ratings.
...
- Craft type includes `Base` alongside `Ground` and `Air`. Base templates require exactly one control unit, may mount weapons (including firearm), reject every engine, and are stationary battlefield fixtures.
- The canonical `standard base` template is a `4x4` square of heavy-steel structure cells with a `1x1` small control unit at the lower-left of the four central cells (`x=1,y=2`).
...
- Base templates are never player/AI deployment candidates: campaign rosters, Test Arena automatic/manual spawning, Craft Arena pair generation, headless arena rosters, and direct deploy APIs exclude them. At battle start, one standard base is created automatically for each side at that side's original fixed base position when the scenario uses bases.
- A base never moves, does not enter the ground capability rule that wrecks craft lacking a usable engine or weapon, and remains valid without a firearm. An armed base uses ordinary AI targeting, aiming, and firing. Losing its control unit starts the same stationary remaining-structure decay lifecycle as a ground-object wreck; it never enters the aircraft drop path. Final base destruction has no special win/loss consequence for now and does not replace the existing rectangular battlefield objective.
- Every live base maintains a circular keep-out boundary computed from the outer corners of its surviving structure cells. No craft may enter that circle: the base stays fixed and overlapping craft are moved to the nearest legal boundary. The radius is recomputed as base structure cells are destroyed, so it is only large enough to contain the remaining base footprint.
...
- Battle simulation defaults to a logical battlefield size of `3000x1500` with `600` ground height (shared authored defaults for browser runtime and headless/arena runs).
...
- `unified-shoot` is a reusable given-target module that uses the live angle-dependent muzzle origin, projectile class, speed, gravity, and full target velocity to compute an exact earliest intercept. Lasers aim at the current target position because their collision is instantaneous; the module applies no identity rules, partial lead, or aim damping.
...
- `npm --prefix arena run verify:shooting-ai` runs a deterministic one-shot proof for every default file-backed weapon: 1,000 random target trajectories per weapon over `0..2000` range, all bearings, and `0..100` target speed, with verification-only unlimited travel, unrestricted firing angles, and zero spread. The exact core must directly hit at least 999 trials per weapon and pass the edge suite. Shooting-AI certification also requires aggregate all-weapon accuracy for unified L1-L5 to remain within +/-1.5 percentage points of 50%/60%/70%/80%/90%; a verifier failure is a release-blocking AI failure.
...
- Built-in composed choices are the explicit certified `L1` through `L5` ladder. Each level has its own certified target and movement family: local engageable threats lead the per-weapon fire list while a short committed strategic target independently drives movement. Shared movement doctrine allows border transit but applies soft recovery after prolonged dwell, holds facing/direction changes, makes ground craft prefer vertical projectile dodges with limited reverse movement, gives aircraft full-vector evasion, and keeps nearby enemies relevant to movement. While engaging, movement holds a standoff at 90% of the currently intended weapon's effective firing range; multi-weapon craft use that one weapon rather than their longest range or a blended craft-wide range. Higher levels improve reaction, compatibility ranking, multi-threat prediction, and coordination. Every certified shoot component continues to delegate to the unchanged `unified-shoot` intercept core and retains aggregate accuracies of 50%/60%/70%/80%/90%.
- Craft classification and every level decision are identity-independent: no template/craft ID or name and no part ID or name is inspected. Roles and tactics derive only from live platform type, speed, acceleration, mass, structure HP/integrity, armor, weapon range/class/damage/penetration/cooldown/capacity/loaded rounds, ballistics, enemy motion, and battlefield position, so newly authored valid craft automatically participate.
- Every level shoot wrapper consumes the target module's given target and independently solves each ready weapon from its live properties. Level accuracy adjustments never inspect craft, template, part ID, or name.
- Ground combat uses the battlefield's full XY plane. AI target ranking and firing eligibility treat vertical separation as ordinary Euclidean distance; weapon range, authored firing arcs, and the ballistic solver determine whether a shot is feasible without a separate ground-Y lane tolerance or penalty.
- Nearby live enemy craft always suppress base targeting and base fire; the opposing base is considered only while every enemy is beyond a property-derived awareness radius. The same rule applies to target movement and each weapon's fallback fire plan.
- `npm --prefix arena run eval:levels -- --minLevel 2 --maxLevel 5` runs 16 deterministic `p4-leaderboard` matches for each requested level against its immediately previous level, alternating which level occupies the Player side. A higher level passes only when its aggregate weighted destroyed-craft total divided by the previous level's total (with a denominator floor of one) is at least `1.10`.
- AI match results and composite-training fitness contain no gas-derived fields. Candidate selection uses Elo (leaderboard phase), Wilson win lower bound, and average destroyed-unit margin in that order. Gameplay gas and deployment inputs remain unchanged.
...
- Leaderboard competition, leaderboard-nearby training, and headless AI evaluation share one `loadLeaderboardScenario()` path. Their battlefield width, height, and ground height are owned by the AI comparison Global Settings, independently of the normal battlefield settings used by browser battles and Test Arena. Comparison-only rules select from every valid non-base runtime craft and maintain the configured live unit count per side by spawning free random craft immediately and after losses. The two sides use independent RNG streams initialized from the same match seed, keeping corresponding random craft selections in the same order. The default target is four units per side, the comparison battlefield is `3000x1500` with `600` ground height, and the game-time duration is 80 seconds.
- The AI Global Settings category owns comparison units per side, battlefield width/height/ground height, simulation duration, and base worth. A comparison continues for the full duration after a base is destroyed. Destroying a craft is worth one destroyed unit and destroying a base is worth 20; no gas, surviving-integrity, remaining-population, or base-HP tiebreaker is used.
- One leaderboard round is one match with model A as Player and model B as Enemy. The symmetric battlefield needs no side-swapped duplicate. The match reports each AI's weighted destroyed total and destroyed/lost ratio, and awards the win only to the larger total.
...
- Leaderboard panel includes quick competition controls: `random pair`, `unranked vs random`, and `manual pair` modes plus configurable run count. Manual-pair rounds cycle the same 16 deterministic seed catalog used by level evaluation.
- Leaderboard model pool contains only the five certified built-ins (`level-1-ai` through `level-5-ai`). Saved training artifacts remain available in AI Selection but are not silently duplicated as built-ins. The persistent version-11 store records Elo, global W/L/T, pair rounds, per-pair results, weighted destroyed/lost totals, and per-match destroyed ratios. The UI shows average destroyed ratio and cumulative destroyed/lost totals for every AI.
- `npm --prefix arena run verify:ai-rules` guards the nearby-enemy base suppression, destroyed-unit-only single-match scoring, and authored AI-comparison settings and battlefield dimensions.
- Leaderboard `Run Competition` submits a batched request and executes rounds in parallel across CPU worker threads (all detected cores when worker runtime is available). While it runs, the panel reports aggregate wall time, queued/active/completed matches, average simulated seconds, and current unit/projectile load from low-frequency worker heartbeats. A scrollable table below the AI leaderboard shows every run's pairing, state, simulated-time progress, unit/projectile load, and wall time; the table is removed when the batch finishes, then the leaderboard/model lists refresh.
