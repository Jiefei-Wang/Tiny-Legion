...
- Built-ins resolve through `levelCompositeConfig(...)`, `game-core/src/ai/composite/level-modules.ts`, and the property-only classifier in `craft-profile.ts`. Target/movement tuples remain L1 `t2/m6`, L2-L3 `t47/m95`, L4 `t47/m97`, and L5 `t47/m99`; their shoot families are now explicitly `unified-level-1-shoot` through `unified-level-5-shoot`. All delegate to `createUnifiedTargetShootAi()`. Each per-shooter/target observer exponentially blends changed velocity using a level response weight; an accuracy gate emits exact plans or an unguided near miss. Near-miss direction and clearance scale are stable for the shooter/target pair, while angular clearance is derived from target radius, projectile radius, predicted intercept distance, and a small margin. Errors over 10 degrees become hesitation. Certification requires aggregate all-weapon targets of 50/60/70/80/90% within +/-1.5 percentage points; standalone `unified-shoot` remains the exact core.
...
- Adjacent built-in level certification is intentionally a single fixed-orientation match per pair because `p4-leaderboard` uses mirrored spawns on a symmetric battlefield. `evaluateAiLevels(...)` runs only `Ln` versus `L(n-1)` and requires `destroyedByHigher / max(1, destroyedByLower) >= 1.10`; it no longer builds a multi-seed round robin or uses win rate as the release criterion.
