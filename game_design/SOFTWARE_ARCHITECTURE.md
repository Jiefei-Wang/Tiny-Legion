...
- Leaderboard compete, `eval-levels`, `eval-tiers`, and phase-four leaderboard-nearby training all consume `arena/src/config/leaderboard-scenario.ts`. It combines the `p4-leaderboard` catalog/base-mode declaration with authored AI comparison settings and shared battlefield defaults, so the Vite endpoint no longer maintains a second scenario parser. Battlefield size, ground height, movement speed multiplier, battle simulation, craft construction, and AI execution therefore use the same game-core code and defaults as Test Arena.
- AI comparison fields in `ai/arena-comparison.yaml` own `spawnCountPerSide=6`, `spawnIntervalSeconds=20`, `maxSimSeconds=100`, and `baseWorthUnits=20`. `run-match.ts` excludes base templates from the random roster, selects one template per wave, and deploys identical free mirrored batches at time zero and each interval with Test-Arena placement and no population cap. `BattleSessionOptions.disableBaseBattleEnd` keeps the fixed-duration comparison active after objective destruction.
...
- `arena/src/eval/verify-ai-rules.ts` asserts awareness-boundary behavior, destroyed-unit-only single-match scoring and ratio math, equality between leaderboard resolution and authored Test Arena/comparison defaults, the symmetric six-unit initial wave, and an actual shared-`BattleSession` parity smoke. `npm --prefix vip run test:headless` runs this guard after the per-template movement/fire smoke.
