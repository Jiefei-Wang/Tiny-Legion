...
- Leaderboard compete and `eval-levels` load `p4-leaderboard` from `arena/composite-training.phases.json` using a working-directory-independent resolver. Browser Test Arena and headless comparison share the authored node defense/base HP values from `ai/arena-comparison.yaml` plus `BATTLEFIELD_WIDTH`, `BATTLEFIELD_HEIGHT`, and `DEFAULT_GROUND_HEIGHT` (currently `3000x1500`, `600` ground height, defense `1.1`, and `1,000,000,000` HP bases). Phase four selects every valid runtime template, starts 10 lane-aware craft per side, and `scenario.maintainUnitsPerSide=10` performs immediate free replacement after each simulation update; paid spawn logic is disabled for this mode. The comparison-only duration remains `120s`.
...
- `arena/src/eval/verify-ai-rules.ts` asserts awareness-boundary behavior, craft-first scoring over a winning base rush, equality between leaderboard resolution and the authored Test Arena defaults, the all-template/10-per-side maintained-population contract, and an actual shared-`BattleSession` parity smoke. `npm --prefix vip run test:headless` runs this guard after the per-template movement/fire smoke.
- Persistent leaderboard storage is schema version `9`; the version bump invalidates ratings produced under the former four-unit, destructible-base comparison scenario. It saves Elo/global results plus per-pair W/L/T. Manual-pair competition cycles deterministic certification seeds, and the leaderboard API/UI auto-loads and displays each level's persisted win rate against its previous level.
