...
- Leaderboard compete starts batched matches in parallel using arena worker threads (`arena/.dist/.../worker-pool.js`) with all detected CPU cores when available, and falls back to single-thread execution if worker runtime is unavailable. The POST returns a job ID immediately; the UI polls `/__arena/composite/leaderboard/compete/status` to show individual match completions, then refreshes ratings after results are applied deterministically in request order.
...
- AI comparison fields in `ai/arena-comparison.yaml` own `spawnCountPerSide=6`, `spawnIntervalSeconds=10`, `maxSimSeconds=600`, and `baseWorthUnits=20`. `run-match.ts` excludes base templates from the random roster, selects one template per wave, and deploys identical free mirrored batches at time zero and each interval with Test-Arena placement and no population cap. `BattleSessionOptions.disableBaseBattleEnd` keeps the fixed-duration comparison active after objective destruction.
