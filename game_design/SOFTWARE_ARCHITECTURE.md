...
- Model ranking prioritizes Elo where applicable, then Wilson win-rate lower bound, then average destroyed-unit margin. AI result types contain no gas-derived fitness fields.
...
- Built-ins resolve through `levelCompositeConfig(...)` to explicit `certified-level-N-target` and `certified-level-N-movement` modules. The target decision carries an independently committed movement attack point and a local-first ranked fire list; unified shooting searches that list per ready weapon without changing its ballistic solver. Movement consumes explicit battlefield/lane bounds and per-unit target, edge-dwell, facing, and ground-direction memory. Its standoff resolver traverses enabled live weapons in the same stable slot order as unified shooting and holds 90% of that single intended weapon's effective range rather than a maximum or aggregate range. Shared doctrine provides soft border recovery, target/facing hysteresis, ground vertical dodging with limited reversal, full-vector air dodging, and local-enemy awareness. Shoot families remain `unified-level-1-shoot` through `unified-level-5-shoot` with unchanged 50/60/70/80/90% calibration.
...
- Leaderboard compete starts batched matches in parallel using arena worker threads (`arena/.dist/.../worker-pool.js`) with all detected CPU cores when available, and falls back to single-thread execution if worker runtime is unavailable. `runMatch(...)` emits an optional progress snapshot every five simulated seconds; `match-worker.ts` forwards those messages through `WorkerPool` without resolving the pending request. The POST returns a job ID immediately, and the UI polls `/__arena/composite/leaderboard/compete/status` for per-match queued/running/final state, simulated time, unit/projectile load, and wall-time diagnostics. Those snapshots populate a transient scrollable run table below the persistent rankings; client state clears the table at batch completion before refreshing ratings after results are applied deterministically in request order.
- Leaderboard compete, `eval-levels`, `eval-tiers`, and phase-four leaderboard-nearby training all consume `arena/src/config/leaderboard-scenario.ts`. It combines the `p4-leaderboard` catalog/base-mode declaration with authored AI comparison settings, so the Vite endpoint no longer maintains a second scenario parser. AI comparison battlefield dimensions are independent from normal browser/Test Arena battlefield defaults while still using the same game-core simulation and craft code.
- AI comparison fields in `ai/arena-comparison.yaml` own `unitsPerSide=4`, `battlefieldWidth=3000`, `battlefieldHeight=1500`, `groundHeight=600`, `maxSimSeconds=80`, and `baseWorthUnits=20`. `run-match.ts` excludes bases from both the random roster and maintained-population count, then maintains the configured live craft population with free random craft. Player and enemy population RNG streams are separate instances initialized from the same match seed, so corresponding replenishments choose the same craft order without sharing mutable RNG state. `BattleSessionOptions.disableBaseBattleEnd` keeps the fixed-duration comparison active after objective destruction.
...
- Each leaderboard round runs one fixed-orientation match with model A as Player and model B as Enemy. `run-match.ts` publishes each side's weighted opposing objects destroyed and finite destroyed/lost ratio; base-unit or objective destruction adds the configured base worth once. `match-comparison.ts` compares those single-match totals. A larger total wins and exact equality ties.
...
- `arena/src/eval/verify-ai-rules.ts` asserts awareness-boundary behavior, destroyed-unit-only single-match scoring and ratio math, equality between leaderboard resolution and authored AI-comparison settings, the symmetric maintained population, and an actual shared-`BattleSession` parity smoke. `npm --prefix vip run test:headless` runs this guard after the per-template movement/fire smoke.
- Persistent leaderboard storage is schema version `11`; the version bump invalidates ratings produced under side-swapped or prior comparison metrics. It saves Elo/global results, per-pair W/L/T, cumulative weighted destroyed/lost counts, and an arithmetic mean of per-match destroyed ratios. Manual-pair competition cycles deterministic certification seeds, and the leaderboard API/UI displays average ratio plus destroyed/lost totals.
- Adjacent built-in level certification runs 16 deterministic matches per pair with alternating Player-side assignment. `evaluateAiLevels(...)` aggregates `Ln` versus `L(n-1)` destroyed totals and requires `destroyedByHigher / max(1, destroyedByLower) >= 1.10`; win rate is not the release criterion.
...
- The Craft Arena heat map uses destroyed count only. A cell displays the row craft's raw destroyed result, while its diverging color is normalized from the signed difference against the column craft. Both mirrored matrix cells point to the same fixed-orientation scenario. Cell selection is presentation state and drives the existing right inspector with both sides' destroyed totals, run metadata, and a rerun action.
...
- `battle-session.ts` (unified command system)
  ...
  - `alive` represents a unit that remains present in battlefield state; `canOperate(unit)` distinguishes active units from timed ground wrecks and falling controller-loss aircraft. `BattleSession.shouldGroundUnitEnterWreck(...)` evaluates the broader ground capability rule (control plus usable ground engine plus fireable weapon), initializes `UnitInstance.groundWreckTimerS`, and makes that countdown irreversible. Aircraft leave the ground-wreck fields unused; bases use them only after control loss, not for missing engine/weapon capability. Active counts, spawning, targeting, collision separation, and arena scoring exclude inoperable units.
  - `UnitType` includes `base`. Template validation rejects engines on bases while retaining the exactly-one-control rule and optional weapon support. `isDeployableTemplate(...)` is the shared roster boundary used by browser and headless arena paths; `BattleSession.deployUnit(...)`, `arenaDeploy(...)`, and automatic candidate selection repeat the guard at the mutation boundary.
  - `STANDARD_BASE_TEMPLATE_ID` resolves the file-backed `standard base` (ID 6), with the in-code initial catalog providing the same fallback. `BattleSession.start(...)` instantiates one per side at the center of the legacy base objective when `spawnBattleBases` is enabled. `UnitInstance.fixedX/fixedY` anchor base position against commands, recoil, impulses, and separation. Armed bases stay in the ordinary target/aim/fire AI pipeline. Control loss enters `groundWreckTimerS` decay while the fixed anchor prevents movement or aircraft-style dropping; missing engines or weapons alone do not start that decay. Objective HP and battle outcome remain intentionally independent for now. Base units are omitted from active-cap counts and loss statistics.
  - `BattleSession.getUnitKeepOutCircle(...)` computes a base's live structure bounding rectangle in world space and returns its centered circumscribed circle; `getUnitKeepOutRadius(...)` exposes the radius alone. Base-versus-craft separation bypasses same-layer filtering and overlap allowance, treats base inverse mass as zero, and applies full positional correction to the other craft. Base pairs are checked directly instead of relying on the ordinary spatial-grid neighbor radius, and inoperable decaying bases retain the boundary until their structure is gone.
...
- `BattleSession.consumeProjectileHitEvents()` exposes bounded structured direct/splash hit telemetry (source unit/weapon, target unit/cell, and projectile class) for renderer-independent verification. `BattleSessionOptions.weaponVerificationOverrides` is an explicit headless-only seam for unlimited projectile travel, unrestricted arcs, zero spread, and disabled unit separation; ordinary matches retain authored rules.
...
- Ground AI target selection and shooting use unrestricted XY geometry. The removed global ground-Y tolerance is not part of target ranking or fire gating; ordinary Euclidean range, per-part firing arcs, and ballistic feasibility own those decisions.
...
- The shared ballistic solver analytically handles zero-gravity relative motion and isolates all positive roots of the gravity trajectory polynomial using the same semi-implicit integration equation as `BattleSession`. Its extended bounded horizon covers the 2,000-unit verification domain. `unified-shoot` iterates the solve against the runtime's angle-dependent muzzle origin, preserves facing for one-shot geometry stability, and special-cases instantaneous lasers.
- `FirePlan.disableTracking` flows through `FireRequest` so an intentional level-accuracy miss cannot be corrected by missile homing; normal plans and manual fire retain authored tracking behavior.
