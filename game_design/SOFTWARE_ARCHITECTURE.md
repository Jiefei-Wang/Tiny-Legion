...
- Developer Tools includes a DOM-only `Craft Arena` screen backed by versioned browser-local scenarios/results. Scenario cards choose two template IDs, independent counts, one composed AI spec, and a mirrored-pair count; deleted template/model references remain visible and block execution until corrected.
- `POST /__arena/craft-arena/simulate` validates a scenario, runs the first mirrored pair as a fail-fast check, then queues remaining same-seed side-swapped matches through the shared worker pool (with sequential single-thread fallback). It returns aggregate pair outcomes and Craft A/B loss totals only and never writes arena data or leaderboard ratings.
...
- Arena `MatchSpec.scenario.initialLineup` optionally selects an exact template/count per runtime side while preserving the legacy symmetric starter path when omitted. `MatchResult.losses` snapshots `BattleSession.getLossStats()`, allowing headless consumers such as Craft Arena to remap destruction and gas loss from runtime sides back to compared craft identities.
