...
- Certified built-ins resolve through `levelCompositeConfig(...)`, `game-core/src/ai/composite/level-modules.ts`, and the property-only classifier in `craft-profile.ts`. L1 preserves the former L2 capability-aware bundle. Remade L2-L6 compose role routing, ammo-cycle movement, urgent threat-vector evasion, spacing, defensive/time-to-disable target ranking, weak-cell fire, history/acceleration prediction, and per-weapon compatible-target selection without craft/part identity checks. The final module tuples are L2 `t2/m6/s5`, L3 `t3/m4/s1`, L4 `t2/m6/s1`, L5 `t3/m6/s4`, and L6 `t3/m6/s7`; `s7` is an internal advanced firing component, not a seventh user-facing AI. The `eval-levels` CLI loads the actual `p4-leaderboard` scenario, scores each seed as a two-game side-swapped series through `mirrored-series.ts`, and exits non-zero unless every higher adjacent level wins strictly more than 60% of 16 series.
...
- Developer Tools includes a DOM-only `Craft Arena` screen backed by versioned browser-local settings, scenarios, and results. The left pane owns global duration, shared composed AI, and Test Arena battlefield geometry. Each compact expandable row owns Craft A Left / Craft B Right template IDs and quantities, and its collapsed summary shows destroyed craft and authored gas waste.
