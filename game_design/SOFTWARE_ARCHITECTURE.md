...
- Built-ins resolve through `levelCompositeConfig(...)`, `game-core/src/ai/composite/level-modules.ts`, and the property-only classifier in `craft-profile.ts`. L1 preserves the former L2 capability-aware bundle. The current tuples are L1 `t2/m6/s1`, L2 `t47/m95/s42`, L3 `t47/m95/s49`, L4 `t47/m97/s54`, and L5 `t47/m99/s93`. Level 5's shoot module preserves the strategic movement target but performs per-weapon kinematic feasibility and firing-angle checks. A slot redirects when the strategic target is infeasible, or when another reachable enemy is substantially closer and its decisive control support fits within that weapon's currently loaded damage budget. Independent slots can therefore fire at different enemies without craft/part identity checks.
